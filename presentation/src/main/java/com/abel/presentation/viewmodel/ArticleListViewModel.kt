...
class ArticleListViewModel(
        ...
        with(viewStates.value) {
            if (this is ArticleListState.ArticlesLoaded || this is ArticleListState.LoadingFailed) {
                val resultsSoFar = when (this) {
                    ...
                    is ArticleListState.ArticlesLoaded -> this.resultsSoFar + (this.pageOfArticles?.articles?.size
                        ?: 0)
                ...
                val totalResults = when (this) {
                    ...
                    is ArticleListState.ArticlesLoaded -> this.pageOfArticles?.totalResults ?: 0
                    else -> throw IllegalStateException("The state ${this.javaClass.simpleName} is unexpected")
                }
                val nextPage = when (this) {
                    is ArticleListState.LoadingFailed -> this.totalPages
                    is ArticleListState.ArticlesLoaded -> (this.pageOfArticles?.page ?: 1) + 1
                    else -> throw IllegalStateException("The state ${this.javaClass.simpleName} is unexpected")
                }
                val articlesLoaded = when (this) {
                    is ArticleListState.ArticlesLoaded -> this.pageOfArticles
    ...
    private fun loadArticleJob(
        ...
        articlesLoaded: PageOfArticles? = null,
        nextPage: Int,
        resultsSoFar: Int,
        totalResults: Int
    ) {
                ...
                _viewStates.value =
                    withContext(backgroundContext) {
                        ...
                        ArticleListState.ArticlesLoaded(
                            ...
                            (nextPageArticles.articles?.size ?: 0) + (articlesLoaded?.articles?.size
                                ?: 0)
                        )
                    }
